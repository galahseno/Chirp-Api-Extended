package com.seno.chirp.api.config

import com.seno.chirp.domain.exception.RateLimitException
import com.seno.chirp.infra.rate_limiting.IpRateLimiter
import com.seno.chirp.infra.rate_limiting.IpResolver
...
class IpRateLimitInterceptor(
    ...
    override fun preHandle(request: HttpServletRequest, response: HttpServletResponse, handler: Any): Boolean {
        if (handler is HandlerMethod && applyLimit) {
            ...
            if (annotation != null) {
                val clientIpAddress = ipResolver.getClientIp(request)
